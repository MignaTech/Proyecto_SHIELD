...
<%@ page import="datos.*, modelos.Agentes" %>
<%@ include file="encabezado.jsp" %>
<%@ include file="barranav.jsp" %>
<%
    Agentes agenUI = (Agentes) request.getSession().getAttribute("perAgente");
    int countAgen1 = new AgenAtkDAO().idAgenAtk(agenUI.getnAgen()).size();
    int countAgen2 = new AgenJuntasDAO().idAgenJuntas(agenUI.getnAgen()).size();
...
<div class="cont-body">
    <c:if test="${sessionScope.currentUser != null}">
        <p>
            Bienvenid@ Agente ${sessionScope.perAgente.nAgen}
        </p>
    </c:if>
    <div class="box-container" style="padding-bottom: 20px">
        <div class="box bg-das" onclick="window.location.href='<%=con%>/Agente3'">
            <div class="text">
                <h2 class="nom-tabla">Editar Perfil</h2>
            </div>
            <img src="<%=con%>/assets/img/usuarios.png">
        </div>
        <div class="box bg-das" onclick="window.location.href='<%=con%>/Agente1'">
            <div class="text">
                <h2 class="cant-tabla"><%= countAgen1 %>
                </h2>
                <h2 class="nom-tabla">Ataques que participas</h2>
            </div>
            <img src="<%=con%>/assets/img/ataque.png">
        </div>
        <div class="box bg-das" onclick="window.location.href='<%=con%>/Agente2'">
            <div class="text">
                <h2 class="cant-tabla"><%= countAgen2 %>
                </h2>
                <h2 class="nom-tabla">Juntas que participas</h2>
            </div>
